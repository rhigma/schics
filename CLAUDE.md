...
| Page | Level |
|---|---|
| `index.php`, `suchen.php`, `ajax_suche.php`, `detail.php`, `alle_versionen.php`, `dashboard.php`, `dashboard_data.php`, `druck.php`, `pdf.php` | READ |
| `neuer_schic.php`, `neue_version.php`, `sortieren.php`, `update_reihenfolge.php` | EDIT |
| `einstellungen.php`, `update_status.php`, `update.php` | ADMIN |

Note: `index.php` is the Fach × Jahrgang overview grid (chips per existing SchiC, plain grey for empty cells). `suchen.php` holds the search form. The actual edit/new-entry surface is `neuer_schic.php`; the admin surface is `einstellungen.php`.
...
- `neuer_schic.php` computes `nextSchicId = MAX(schic_id) + 1` inside an explicit `beginTransaction()` to avoid the read-then-insert race. The `reihenfolge` is also computed server-side (`MAX(reihenfolge)+1` within the same `fach`/`jahrgang`) — never let it come from the form.
...
- Hausschrift ist **Berlin Type** (Berliner Hausschrift). Web-Varianten (woff2/woff) und Office-TTFs für mPDF liegen unter `assets/fonts/`, die Web-Varianten werden via `@font-face` in `style.css` registriert, die TTFs registriert `pdf.php` über `fontDir`/`fontdata`. Bei Layout-Anpassungen die Schrift beibehalten, nicht durch eine Systemschrift ersetzen.
...
- Don't link `sortieren.php` or `neuer_schic.php` from non-edit users' nav, and don't expose `einstellungen.php` to editors. `nav.php` already gates these — keep it that way.
